...
export function Header() {
  ...
  return (
    <header style={{ position: 'relative', display: 'flex', alignItems: 'center', flexWrap: 'wrap', padding: '14px 20px', borderBottom: '1px solid var(--border)', gap: 12, zIndex: 10 }}>
      <div style={{ position: 'absolute', bottom: -1, left: 20, width: 460, maxWidth: '80vw', height: 1, background: 'linear-gradient(90deg, var(--amber), transparent)' }} />
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 25, letterSpacing: '0.08em', color: 'var(--amber)', flexShrink: 0 }}>ALGO<span style={{ color: 'var(--text-dim)' }}>SPHERE</span></div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '0.12em', padding: '4px 9px', border: `1px solid ${statusColor}`, color: statusColor, flexShrink: 0 }}>
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: statusColor }} />OMNI PAPER RESEARCH · {status.toUpperCase()}
      </div>
      <nav style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
        {[
          ...
          <NavLink key={to} to={to} style={({ isActive }) => ({ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '0.14em', padding: '5px 9px', color: isActive ? 'var(--amber)' : 'var(--text-muted)', background: isActive ? 'var(--amber-dim)' : 'transparent', border: `1px solid ${isActive ? 'var(--amber-dim)' : 'transparent'}`, textDecoration: 'none' })}>{label}</NavLink>
        ))}
      </nav>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        {userId && <><span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-dim)', border: '1px solid var(--border)', padding: '4px 8px' }}>{role === 'admin' ? '★ ' : ''}{userId} · PAPER</span><button onClick={handleLogout} style={buttonStyle}>OUT</button></>}
...
const buttonStyle = { fontFamily: 'var(--font-mono)', fontSize: 9, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', padding: '4px 8px', cursor: 'pointer' }
