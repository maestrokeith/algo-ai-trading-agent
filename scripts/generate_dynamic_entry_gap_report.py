#!/usr/bin/env python3
"""Generate research-only dynamic universe to entry-eval gap diagnostics."""
...
from src.dynamic_entry_gap_report import (  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Research-only diagnostics for selected dynamic symbols lost before ENTRY_EVAL."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"DYNAMIC_ENTRY_GAP_REPORT_FAILED {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    print(render_dynamic_entry_gap_report(report))
