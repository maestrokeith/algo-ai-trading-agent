#!/usr/bin/env python3
"""Generate research-only dynamic threshold miss diagnostics."""
...
from src.dynamic_threshold_miss import (  # noqa: E402
...
def _journal_log_path(day: str, unit: str) -> Path | None:
    if not unit or shutil.which("journalctl") is None:
        return None
    path = Path("/tmp") / f"dynamic_threshold_miss_{day}_{unit.replace('.', '_')}.log"
    cmd = [
        "journalctl",
        "-u",
        unit,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Research-only dynamic scanner threshold miss report.")
    parser.add_argument("--date", required=True, help="Trading date YYYY-MM-DD.")
    parser.add_argument("--user", default="live_bot", help="User id for input selection and output naming.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"DYNAMIC_THRESHOLD_MISS_FAILED {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    print(render_dynamic_threshold_miss_report(report))
